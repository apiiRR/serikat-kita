...
function PhotoEditor({
  ...
  albums: GalleryAlbum[];
  busy: boolean;
  onSave: (caption: string, album: string) => void;
  onDelete: () => void;
}) {
    ...
    <Card>
      <CardContent className="p-4 space-y-3">
        ...
        <div className="flex gap-2">
          <Button disabled={busy} onClick={() => onSave(caption, album)}>
            Simpan
          </Button>
...
export default function AdminGallery() {
  ...
  async function mutate(action: () => Promise<void>, message: string) {
    ...
    try {
      ...
      toast({
        title: "Gagal",
        description: "Perubahan gagal disimpan. Silakan coba lagi.",
        variant: "destructive",
      });
    } finally {
      setBusy(false);
  ...
  function removePending(item: PendingPhoto) {
  ...
  async function upload() {
    setBusy(true);
    const update = (id: string, changes: Partial<PendingPhoto>) =>
      setQueue((current) =>
        current.map((item) =>
    ...
    <div className="space-y-8">
      ...
      >
          ...
          onSubmit={(e) => {
            ...
            void mutate(async () => {
              const result = editingAlbum
                ? await supabase
                    .from("gallery_albums")
                    .update({ name: albumName.trim() })
                    .eq("id", editingAlbum)
                : await supabase
                    .from("gallery_albums")
                    .insert({ name: albumName.trim() });
        ...
        >
          ...
          />
          <Button type="submit">
            {editingAlbum ? "Simpan nama" : "Tambah album"}
          </Button>
          {editingAlbum && (
            <Button
              type="button"
        ...
        <div className="space-y-2">
          {albums.map((album) => (
            <div
              key={album.id}
              className="flex flex-wrap items-center gap-2 border rounded p-3"
            >
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    !confirm(
                      `Hapus album “${album.name}”? Foto tetap tersimpan tanpa album.`,
                    )
                  )
                    return;
                  void mutate(async () => {
                    const result = await supabase
                      ...
                      .eq("id", album.id);
                    if (result.error) throw result.error;
                    if (uploadAlbum === album.id) setUploadAlbum("");
                    if (editingAlbum === album.id) {
                      setEditingAlbum(null);
                      setAlbumName("");
                    }
                  }, "Album dihapus; foto tetap tersimpan.");
                }}
              >
                Hapus album
              </Button>
      ...
      >
        ...
        <p className="text-sm text-muted-foreground">
          JPEG, PNG, atau WebP, maksimal 5 MB per foto. Foto langsung tampil di
          website setelah berhasil diunggah.
        </p>
        <AlbumSelect
          albums={albums}
          value={uploadAlbum}
          onChange={setUploadAlbum}
          label="Album tujuan upload"
        />
        <Input
          type="file"
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {queue.map((item) => (
            <div key={item.id} className="border rounded-lg p-3 space-y-2">
              ...
              <Input
                aria-label={`Keterangan ${item.file.name}`}
              ...
              <p role="status" className="text-sm">
                {
                  {
                    pending: "Siap diunggah",
                    uploading: "Mengunggah...",
                    success: "Berhasil diunggah",
                    error: "Gagal diunggah",
                  }[item.status]
                }
              </p>
              {item.error && (
        ...
        >
          {busy ? "Memproses..." : "Unggah / coba ulang foto gagal"}
        </Button>
      </fieldset>
      <div>
        ...
        {loading ? (
          ...
          <p className="text-muted-foreground">
            Belum ada foto. Tambahkan foto untuk memulai.
          </p>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {photos.map((photo) => (
              <PhotoEditor
                ...
                onSave={(caption, album) =>
                  void mutate(async () => {
                    const result = await supabase
                      .from("gallery_photos")
                      .update({ caption, album_id: album || null })
                      .eq("id", photo.id);
                    if (result.error) throw result.error;
